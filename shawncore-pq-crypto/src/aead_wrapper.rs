...
pub(crate) const CHACHA20_MAX_BYTES: u64 = u32::MAX as u64 * 64;
...
pub fn hmac_sha384(key: &[u8], data: &[u8]) -> Result<[u8; AEAD_TAG_SIZE], CryptoError> {
    ...
    mac.update(data);
    let mut result_bytes = mac.finalize().into_bytes();

    let mut output = [0u8; AEAD_TAG_SIZE];
    output.copy_from_slice(&result_bytes);
    secure_zeroize(result_bytes.as_mut_slice());
...
pub fn aead_encrypt(
    ...
    let mut result_bytes = mac_engine.finalize().into_bytes();
...
pub fn aead_decrypt(
    ...
    let mut expected_mac_bytes = mac_engine.finalize().into_bytes();
