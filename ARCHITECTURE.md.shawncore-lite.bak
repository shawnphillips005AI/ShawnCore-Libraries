...
## 13. USV Deployment Profile

The workspace provides a dedicated Cargo `usv` profile for constrained onboard
firmware builds. The profile inherits the existing `release` profile instead of
duplicating its settings, keeping the release hardening as the single source of
truth while adding `opt-level = "z"` to prioritize code-size reduction.

This is a build-time optimization profile only. It is **not** a claim of
hardware readiness, safety certification, cryptographic certification, or
production deployment readiness.

Before deployment on actual vehicle hardware, the target integration must
validate at minimum:

- flash and RAM budgets;
- stack usage and worst-case execution time;
- interrupt and scheduler latency;
- hardware-watchdog timing;
- target-specific DMA/cache behavior;
- cryptographic KAT/interoperability behavior;
- hardware entropy behavior;
- target-specific concurrency behavior; and
- independent security requirements.

Build with:

```text
cargo build --workspace --profile usv
```

### Watchdog integration

The watchdog is already implemented as a host/RTOS callback rather than a
cryptographic-library callback. The C ABI exposes
`shawncore_rtos_register_pet_watchdog()`, and the RTOS scheduler owns the
decision to invoke the registered callback.

The USV integration layer should connect this callback to a short,
non-blocking hardware-watchdog service routine. That routine must not perform
cryptographic work, allocate, block, acquire long-duration locks, or otherwise
interfere with scheduler progress.

ShawnCore deliberately does not insert watchdog calls into post-quantum
cryptographic primitives. This keeps watchdog policy platform-specific and
avoids coupling cryptographic timing to RTOS control flow.

### Entropy mixing

The watchdog interface is intentionally separate from entropy-pool mixing.
Entropy collection and mixing must preserve the existing bounded critical
section: expensive SHA-384 work should not be performed while the
interrupt-disabled pool-state lock is held.

A target platform may schedule bounded entropy collection/mixing from an
appropriate task or other platform-controlled context according to its
hardware entropy source and timing requirements.
